...
for %%F in (*.txt) do (
    ...
    for /f "tokens=4 delims=_" %%A in ("%%F") do (
        set "date_part=%%A"
    ...
    REM Create the new filename in MMDD format
    call set "new_filename=%%month_day%%.txt"

    REM Check for duplicate files and append a counter if necessary
    set /a counter=0
    if exist "!new_filename!" (
        set /a counter+=1
        set "new_filename=!month_day!_!counter!.txt"
    )
